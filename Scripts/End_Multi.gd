...
func _render_score(player):
	if Global.enable_combo:
		...
		var _t = "Player %s\n\nGrade: %s\nScore: %s \nGreat notes: %s \nGood notes: %s \nOkay notes: %s \nMissed notes: %s"
		if player == 1:
			$VBoxContainer/HBoxContainer/Player1/Label.text = _t % [str(player), Global.grade_p1, Global.score_p1, Global.great_p1, Global.good_p1, Global.okay_p1, Global.missed_p1]
		elif player == 2:
			$VBoxContainer/HBoxContainer/Player2/Label.text = _t % [str(player), Global.grade_p2, Global.score_p2, Global.great_p2, Global.good_p2, Global.okay_p2, Global.missed_p2]
